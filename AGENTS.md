...
You are inside the LFX Skills source repository. agents.md-compatible tools use the `cli/lfx-skills` CLI to install skills globally or per repo. Claude Code uses the `.claude-plugin/plugin.json` plugin manifest and does not use the CLI installer. The four helper skills live under `skills/` for clone/agent workflows and are intentionally not in the Claude plugin.
