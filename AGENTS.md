...
You are inside the LFX Skills source repository. agents.md-compatible tools use the `cli/lfx-skills` CLI to install skills globally or per repo. Claude Code uses the `.claude-plugin/plugin.json` plugin manifest and does not use the CLI installer.
