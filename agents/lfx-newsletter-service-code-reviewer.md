...
| Send handoff, `groupId`, email-service analytics, NATS/email publication | Check `docs/recipient-resolution.md`, `docs/newsletter-service-contract.md`, and, if present, `lfx-v2-email-service/docs/email-service-contract.md`. Current service behavior is a state transition only; do not require direct email dispatch unless the repo docs changed to say so. |
