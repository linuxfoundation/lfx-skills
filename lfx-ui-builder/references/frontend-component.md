...
## Class Structure (from component-organization.md rule)
...
| ------------------ | ---------------------------- | ------------------------- | ---------------------------------------- |
| `signal()`         | `@angular/core`              | Simple writable state     | `loading = signal(false)`                |
| `input()`          | `@angular/core`              | Parent → child (optional) | `label = input<string>('Default')`       |
| `input.required()` | `@angular/core`              | Parent → child (required) | `itemId = input.required<string>()`      |
| `output()`         | `@angular/core`              | Child → parent events     | `saved = output<Item>()`                 |
| `computed()`       | `@angular/core`              | Derived from signals      | `total = computed(() => items().length)` |
| `model()`          | `@angular/core`              | Two-way binding           | `visible = model(false)`                 |
| `toSignal()`       | `@angular/core/rxjs-interop` | Observable → signal       | `toSignal(obs$, { initialValue: [] })`   |
| `toObservable()`   | `@angular/core/rxjs-interop` | Signal → observable       | `toObservable(this.visible)`             |
...
- Use pipes for template transforms — never methods (performance + caching)
- For PrimeNG wrappers, use `descendants: false` on `@ContentChild` — this is critical
...
## Drawer-Specific Conventions

When building a drawer component, also apply these patterns:

- `model<boolean>(false)` for visibility with two-way binding
- Lazy loading via `toObservable(visible).pipe(skip(1), switchMap(...))`
- `forkJoin` for parallel API calls inside the `switchMap`
- Chart data as `computed()` signals, options as static `protected readonly`
- Responsive width: `xl:w-[45%] lg:w-[55%] md:w-[70%] sm:w-[90%] w-full`

See the `/ui` skill SKILL.md Section 3 for full drawer pattern details.
