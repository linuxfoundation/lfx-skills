...
#
# The file is idempotent and respects existing values: LFX_DEV_ROOT is set
# only if not already set (so a user's session-level override wins), and
# PATH is appended only if not already on it. Safe to source repeatedly,
# both from the user's shell rc and inline at the top of skill bash blocks.
write_env_sh() {
  ...
  cat > "$env_path" <<EOF
# Generated by lfx-skills install. Idempotent: safe to source from shell rc
# AND inline from skill bash blocks (skills source it for LFX_DEV_ROOT).
# DO NOT EDIT — re-run \`lfx-skills install\` or \`lfx-skills config set\` to regenerate.
: "\${LFX_DEV_ROOT:=$devroot}"
export LFX_DEV_ROOT
case ":\$PATH:" in
  *":$bin_path:"*) ;;
